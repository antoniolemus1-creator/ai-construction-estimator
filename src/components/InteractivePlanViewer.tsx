...
export function InteractivePlanViewer({ planId, pdfUrl, onMeasurement }: InteractivePlanViewerProps) {
            ...
            try {
              ...
              if (dims.coordinates) {
                if (dims.coordinates.points) {
                  // Wall - has start/end points
                  coordinates = dims.coordinates.points.map((p: {x: number, y: number}) => ({ x: p.x, y: p.y }));
                } else if (dims.coordinates.width && dims.coordinates.height) {
                  // Ceiling - has bounding box
  ...
  // Render takeoff overlays and markups
  const renderOverlays = useCallback(() => {
    ...
    // Render takeoff highlights
    if (showTakeoff) {
      for (const item of takeoffItems) {
        if (!item.visible || !item.coordinates) continue;

        ctx.strokeStyle = item.color || '#3B82F6';
        ctx.fillStyle = `${item.color}33` || '#3B82F633'; // 20% opacity
        ctx.lineWidth = 3;

        for (const coord of item.coordinates) {
          if (coord.points && coord.points.length > 1) {
            // Draw polyline (for walls)
            ctx.beginPath();
            ctx.moveTo(coord.points[0].x * scale, coord.points[0].y * scale);
            for (let i = 1; i < coord.points.length; i++) {
              ctx.lineTo(coord.points[i].x * scale, coord.points[i].y * scale);
            }
            ctx.stroke();
          } else if (coord.width && coord.height) {
            // Draw rectangle (for rooms, doors, windows)
            ctx.fillRect(coord.x * scale, coord.y * scale, coord.width * scale, coord.height * scale);
            ctx.strokeRect(coord.x * scale, coord.y * scale, coord.width * scale, coord.height * scale);
          } else {
            // Draw marker (for fixtures)
            ctx.beginPath();
            ctx.arc(coord.x * scale, coord.y * scale, 10, 0, Math.PI * 2);
            ctx.fill();
            ctx.stroke();
          }
        }
  ...
  }, [takeoffItems, markups, currentMarkup, showTakeoff, showMarkups, scale, measurementStart, activeTool]);
  ...
  // Group takeoff items for legend
  const groupedItems = takeoffItems.reduce((acc, item) => {
    ...
    return acc;
  }, {} as Record<string, { itemType: string; wallType: string | null; color: string; items: TakeoffItem[]; totalQuantity: number; unit: string; visible?: boolean }>);
            ...
            {Object.entries(groupedItems).map(([key, group]) => (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: group.visible ? group.color : 'transparent', borderColor: group.color }}
                />
